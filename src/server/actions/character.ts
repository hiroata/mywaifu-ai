...
const characterSchema = z.object({
  ...
  shortDescription: z.string().max(100, "短い説明は100文字以内で入力してください").optional(),
  age: z.number().int().min(18, "年齢は18歳以上で入力してください").max(100, "年齢は100歳以下で入力してください").optional(),
  gender: z.enum(["male", "female", "other"]),
  type: z.enum(["real", "anime"]),
  personality: z.string().min(10, "性格は10文字以上で入力してください").max(500, "性格は500文字以内で入力してください"),
...
export async function createCustomCharacter(formData: FormData) {
  ...
  try {
    ...
    if (!subscription || subscription.plan === "FREE") {
      return {
        success: false,
        error: "カスタムキャラクターの作成にはプレミアムまたはアルティメットプランへのアップグレードが必要です",
      };
    }
    ...
    if (subscription.plan === "PREMIUM") {
      ...
      if (customCharacterCount >= 3) {
        return {
          success: false,
          error: "プレミアムプランでは最大3体のカスタムキャラクターを作成できます。無制限の作成にはアルティメットプランへのアップグレードが必要です",
        };
      }
    }
...
export function syncServerStateWithStore(characterId: string, action: "add" | "update" | "delete" | "favorite", data?: any) {
  try {
    // SSRの場合はスキップ
